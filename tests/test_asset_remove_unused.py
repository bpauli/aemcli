...
class TestAssetRemoveUnused:
    ...
    def setup_method(self):
        ...
        self.test_data_dir = (
            Path(__file__).parent / "test_content" / "asset_remove_unused"
        )
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        <metadata
            ...
            assert result.exit_code == 0
            assert (
                "The following unused asset folders will be PERMANENTLY DELETED:"
                in result.output
            )
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:cq="http://www.day.com/jcr/cq/1.0"
    ...
    def test_find_all_content_xml_files_function(self):
        ...
        assert (
            len(content_files) >= 4
        )  # assets + pages + components + page_with_reference
    ...
    def test_find_asset_content_xml_files_function(self):
        ...
        asset_data = [
            (asset_path, mime_type) for _, asset_path, mime_type in asset_files
        ]
        ...
        assert (
            any("/content/dam/uncommon/asset3" in path for path, _ in asset_data)
            == False
        )
    ...
    def test_help_output(self):
        ...
        assert result.exit_code == 0
        assert (
            "Find and remove unused DAM assets with common MIME types" in result.output
        )
        assert (
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            assert (
                check_folder_thumbnail_paths(
                    "test_folder/.content.xml", "/content/dam/asset2"
                )
                == True
            )
            ...
            assert (
                check_folder_thumbnail_paths(
                    "test_folder/.content.xml", "/content/dam/nonexistent"
                )
                == False
            )
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            # Clean one asset
            result = clean_folder_thumbnail_paths(
                "test_folder/.content.xml", "/content/dam/asset2"
            )
            assert result == True
            ...
            assert (
                check_folder_thumbnail_paths(
                    "test_folder/.content.xml", "/content/dam/asset1"
                )
                == True
            )
            assert (
                check_folder_thumbnail_paths(
                    "test_folder/.content.xml", "/content/dam/asset3"
                )
                == True
            )
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            result = clean_folder_thumbnail_paths(
                "test_folder/.content.xml", "/content/dam/only-asset"
            )
            assert result == True
    ...
    def test_asset_with_folder_thumbnail_paths_references(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            # Create a DAM asset
            with open(
                "test_dir/jcr_root/content/dam/thumbnail_only/.content.xml", "w"
            ) as f:
                f.write(
                    """<?xml version="1.0" encoding="UTF-8"?>
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            assert result.exit_code == 0
            assert (
                "THUMBNAIL ONLY: /content/dam/thumbnail_only (MIME: image/jpeg)"
                in result.output
            )
            ...
            assert check_folder_thumbnail_paths(
                "test_dir/jcr_root/content/folder/.content.xml",
                "/content/dam/thumbnail_only",
            )
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            assert check_folder_thumbnail_paths(
                "test_dir/jcr_root/content/folder/.content.xml", "/content/dam/to_clean"
            )
            ...
            assert result.exit_code == 0
            assert (
                "THUMBNAIL ONLY: /content/dam/to_clean (MIME: image/png)"
                in result.output
            )
            assert (
                "folderThumbnailPaths references will be cleaned up first"
                in result.output
            )
            assert "Cleaning folderThumbnailPaths references:" in result.output
            assert (
                "CLEANED: /content/dam/to_clean - removed from 1 folderThumbnailPaths"
                in result.output
            )
            assert "Cleaned 1 folderThumbnailPaths references" in result.output

            # Verify the reference was cleaned up
            assert not check_folder_thumbnail_paths(
                "test_dir/jcr_root/content/folder/.content.xml", "/content/dam/to_clean"
            )

            # Verify other references remain
            assert check_folder_thumbnail_paths(
                "test_dir/jcr_root/content/folder/.content.xml", "/content/dam/other"
            )
            assert check_folder_thumbnail_paths(
                "test_dir/jcr_root/content/folder/.content.xml", "/content/dam/another"
            )
    ...
    def test_asset_with_folder_thumbnail_paths_cancellation(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            # Create a DAM asset
            with open(
                "test_dir/jcr_root/content/dam/cancelled_clean/.content.xml", "w"
            ) as f:
                f.write(
                    """<?xml version="1.0" encoding="UTF-8"?>
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
...
<jcr:root xmlns:jcr="http://www.jcp.org/jcr/1.0" xmlns:dam="http://www.day.com/dam/1.0" xmlns:nt="http://www.jcp.org/jcr/nt/1.0"
    ...
    <jcr:content
        ...
        jcr:primaryType="nt:unstructured"/>
            ...
            assert result.exit_code == 0
            assert (
                "THUMBNAIL ONLY: /content/dam/cancelled_clean (MIME: image/png)"
                in result.output
            )
            assert (
                "folderThumbnailPaths references will be cleaned up first"
