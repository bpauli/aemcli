...
def check_asset_references(asset_path, content_xml_files):
    ...
    for xml_file in content_xml_files:
        try:
            ...
            if (
                f'image="{asset_path}"' in content
                or f"image='{asset_path}'" in content
                or f'fileReference="{asset_path}"' in content
                or f"fileReference='{asset_path}'" in content
            ):
...
def asset_remove_unused(path, dry_run):
    ...
    click.echo(
        f"Found {len(all_content_xml_files)} .content.xml files to check for references"
    )
    click.echo("-" * 70)
    ...
    for file_path, asset_path, mime_type in asset_files:
        ...
        if references["files"]:
            ...
            click.echo(
                f"USED: {asset_path} (MIME: {mime_type}) - {len(references['files'])} reference(s)"
            )
        elif references["thumbnail_refs"]:
            # Asset is only used in folderThumbnailPaths - mark for cleanup and deletion
            assets_with_thumbnail_refs.append(
                (file_path, asset_path, mime_type, references["thumbnail_refs"])
            )
            click.echo(
                f"THUMBNAIL ONLY: {asset_path} (MIME: {mime_type}) - {len(references['thumbnail_refs'])} folderThumbnailPaths reference(s)"
            )
    ...
    if assets_with_thumbnail_refs:
        click.echo(
            f"  Assets with only thumbnail references: {len(assets_with_thumbnail_refs)}"
        )
    ...
    if unused_assets:
        ...
        if dry_run:
            ...
            if assets_with_thumbnail_refs:
                click.echo(
                    "(Note: folderThumbnailPaths references will be cleaned up first)"
                )
            ...
            if assets_with_thumbnail_refs:
                click.echo(
                    f"Would clean {len(assets_with_thumbnail_refs)} folderThumbnailPaths references"
                )
        else:
            ...
            if assets_with_thumbnail_refs:
                click.echo(
                    f"Note: {len(assets_with_thumbnail_refs)} folderThumbnailPaths references will be cleaned up"
                )
            click.echo()

            # Ask for confirmation
            if click.confirm(
                "Do you want to proceed with the deletion?", default=False
            ):
                click.echo("-" * 70)
                ...
                if assets_with_thumbnail_refs:
                    ...
                    for (
                        ...
                        mime_type,
                        thumb_files,
                    ) in assets_with_thumbnail_refs:
                        cleaned_count = 0
                        for thumb_file in thumb_files:
                            if clean_folder_thumbnail_paths(thumb_file, asset_path):
                                cleaned_count += 1
                        click.echo(
                            f"  CLEANED: {asset_path} - removed from {cleaned_count} folderThumbnailPaths"
                        )
                        total_cleaned += cleaned_count
                    click.echo(
                        f"  Total: Cleaned {total_cleaned} folderThumbnailPaths references"
                    )
                    click.echo("-" * 70)
                ...
                if assets_with_thumbnail_refs:
                    click.echo(
                        f"Cleaned {len(assets_with_thumbnail_refs)} folderThumbnailPaths references"
                    )
            else:
