...
class PackageBuilder:
    ...
    def create_package(self, temp_dir, filter_path, package_group, package_name, package_version):
        ...
        placeholder_file.write_text("# Placeholder file to ensure jcr_root directory is included in package\n")
...
def validate_jcr_root(path):
    ...
    if 'jcr_root' not in str(path_obj):
        raise click.UsageError(f"Not inside a vault checkout with a jcr_root base directory: {path}")
    ...
    filter_path = '/' + '/'.join(parts[jcr_root_index + 1:]) if len(parts) > jcr_root_index + 1 else '/'
...
@click.option('-u', '--credentials', help='User credentials in format user:password (default: admin:admin)')
...
def checkout(jcr_path, server, credentials, force, quiet):
    ...
    if jcr_path == '/':
        raise click.UsageError("Refusing to work on repository root (would be too slow or overwrite everything)")
    ...
    ctx.invoke(get, path=local_path, server=server, credentials=credentials, force=force, quiet=quiet)
...
def put(path, server, credentials, force, quiet):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        builder.create_package(temp_dir, filter_path, config.package_group, package_name, package_version)
        
        # Copy content to package
        jcr_root_dest = os.path.join(temp_dir, 'jcr_root')
        filter_dirname = os.path.dirname(filter_path)
        if filter_dirname != '/':
            os.makedirs(os.path.join(jcr_root_dest, filter_dirname.lstrip('/')), exist_ok=True)
        ...
        if not config.quiet:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                jcr_files = [f for f in zf.namelist() if f.startswith(f'jcr_root{filter_path}')]
        ...
        # Upload and install
        try:
            package_path = f"{config.package_group}/{package_name}-{package_version}.zip"
        ...
        try:
            ...
            if os.path.exists(source_path):
                ...
                if not config.quiet:
                    ...
                    try:
                        git_cwd = os.path.dirname(path) if os.path.dirname(path) else '.'
...
def status(path, server, credentials):
    ...
    if filter_path == '/':
        raise click.UsageError("Refusing to work on repository root (would be too slow)")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            filter_dirname = os.path.dirname(filter_path)
            if filter_dirname != '/':
                os.makedirs(os.path.join(local_dir, filter_dirname.lstrip('/')), exist_ok=True)
...
def _diff_command(path, server, credentials, inverse=False):
    ...
    click.echo(f"Showing differences ({diff_type}) for {filter_path} against {config.server}")
